...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) { // 启动或者重启raft服务器
	cfg.crash1(i)
...
func (cfg *config) checkTerms() int { // 检测所有节点是否同意当前任期
	term := -1
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
...
func (cfg *config) nCommitted(index int) (int, interface{}) { // 多少机器认为当前log已提交
	count := 0
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int { // 进行命令提交操作， retry会进行多次重试，并检测是否被raft主节点commit
	t0 := time.Now()
	starts := 0
	for time.Since(t0).Seconds() < 10 {
		// try all the servers, maybe one is the leader.
		index := -1
		for si := 0; si < cfg.n; si++ { // 客户端不知道那个服务器是leader，依次找leader
			...
			cfg.mu.Unlock()
			if rf != nil {
				index1, _, ok := rf.Start(cmd) // 给raft集群发送日志
				if ok {
					index = index1
					break
				}
