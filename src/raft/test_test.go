...
func TestInitialElection2A(t *testing.T) { // 测试初始leader的产生
	servers := 3 // 3个raft服务器
...
func TestReElection2A(t *testing.T) { // 测试删除leader后产生新leader
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
	...
	// disturb the new leader. 老领导来了要遵循新领导
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected. 人数不够，没有领导
...
func TestManyElections2A(t *testing.T) { // 随机宕机三个，寻找新leader，重复10次
	servers := 7
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestBasicAgree2B(t *testing.T) { // 测试一次简单提交（重复三次），流程：使用start提交一个日志，判断日志是否被集群commit
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
	...
	for index := 1; index < iters+1; index++ {
		nd, _ := cfg.nCommitted(index) // 多少节点认为改日志已提交
		if nd > 0 {
			t.Fatalf("some have committed before Start()") // 还没发送就检测到已提交
		}
...
func TestRPCBytes2B(t *testing.T) { // rpc字节计数，每个命令只发送一次
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestFailAgree2B(t *testing.T) { // 断开一个follower，然后发送写操作，然后重新启动这个follower
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestFailNoAgree2B(t *testing.T) { // 5个节点，宕机其中三个非leader节点，然后发送请求判断是否被提交（正常leader不能提交这个请求），然后恢复在发送一个新请求
	servers := 5
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestConcurrentStarts2B(t *testing.T) { // 测试连续的发送命令
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
loop: // 方便多层循环的跳出
	for try := 0; try < 5; try++ {
		if try > 0 {
			// give solution some time to settle 给一些时间
		...
		for index := range is {
			...
			if ix, ok := cmd.(int); ok {
				if ix == -1 {
					// peers have moved on to later terms 部分出现新term，不是全部start都会成功
		...
		if failed {
			// avoid leaking goroutines 把通道数据读取干净
			go func() {
				for range is {
				}
			}()
...
func TestRejoin2B(t *testing.T) { // 老leader宕机后，收到一些数据，新leader也收到数据，老leader重新连接
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestBackup2B(t *testing.T) { // ①是原始leader，发送一个请求，关掉③④⑤，发送一些不能commit的请求，关闭①②，打开③④⑤提交一些请求，打开①③发送一些请求。。。
	servers := 5
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
...
func TestCount2B(t *testing.T) { // 测试是否有太多不必要的rpc请求
	servers := 3
	cfg := make_config(t, servers, false, false)
	defer cfg.cleanup()
