...
type Raft struct {
	...
	replicatorCond []*sync.Cond // used to signal replicator goroutine to batch replicating entries 批量复制条目
...
func (rf *Raft) persist() { // 进行持久化
	rf.persister.SaveRaftState(rf.encodeState())
}

// restore previously persisted state.
func (rf *Raft) readPersist(data []byte) { // 持久化读取
...
func (rf *Raft) ChangeState(state NodeState) {
	...
	switch state {
	case StateFollower:
		rf.heartbeatTimer.Stop() // 不是leader不发心跳
		rf.electionTimer.Reset(RandomizedElectionTimeout())
	case StateCandidate:
	case StateLeader:
		lastLog := rf.getLastLog() // 让其他服务器遵循自己的log
		for i := 0; i < len(rf.peers); i++ {
			rf.matchIndex[i], rf.nextIndex[i] = 0, lastLog.Index+1
		}
		rf.electionTimer.Stop() // leader发心跳，不重新选举
