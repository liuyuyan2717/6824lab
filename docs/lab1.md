Note: The repository's current src/mr implementation contains the lab template (skeleton) — Worker, Coordinator RPC handlers and related functions are not fully implemented in src/mr/*. The examples and reference implementation shown below are a worked example you can follow to implement the missing pieces. To run the lab as described, implement the functions in:

- src/mr/worker.go
- src/mr/coordinator.go
- src/mr/rpc.go

The rest of this document describes a reference implementation and example code.
...
```
(omitted here for brevity in-doc; see the reference implementation below in the repo or implement in src/mr/*. )
```

#### rpc.go

```
(omitted here for brevity; see the reference implementation below in the repo or implement in src/mr/*. )
```
